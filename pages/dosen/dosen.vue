<template>
    <div>
      <div class="container mx-auto">
        <p class="font-semibold text-center mt-2 mb-5">Dosen</p>
        <NuxtLink
          to="/dosen/addosen"
          ...
          <span class="mt-1 ml-1"> Add data dosen </span>
        </NuxtLink>
          ...
          <table class="w-full text-sm text-left text-slate-500">
            <thead
              ...
              <tr>
                ...
                <th scope="col" class="px-6 py-4">Nama Dosen</th>
                <th scope="col" class="px-6 py-4">Matakuliah</th>
                <th scope="col" class="px-6 py-4">Aksi</th>
              </tr>
            </thead>
            <tbody id="example">
              <tr
                v-for="(row, index) in nama_dosen"
                :key="row.addosen"
                class="bg-white border-b hover:bg-slate-100 text-center">
                <td class="px-6 py-4">{{ index + 1 }}</td>
                <td class="px-6 py-4">{{ row.adddosen }}</td>
                <td class="px-6 py-4">
                  <a :href="'/matakuliah/' + row.id">{{ row.nama_dosen }}</a>
                </td>
                <td class="px-6 py-4">{{ row.dosen }}</td>
                <td class="px-3 py-2 mx-auto">
                  <div class="grid lg:grid-cols-2 md:grid-cols-1 w-28 mx-auto">
                    <NuxtLink
                      :to="'/dosen/update/' + row.adddosen"
                    ...
                    <button
                      type="button"
                      @click="($event) => deletenama_dosen($event, row.adddosen)"
  ...
  <script>
    ...
    const BaseUrl = "https://64abc8e39edb4181202e87b6.mockapi.io/akademik/v1";
    export default {
      name: "row",
      data() {
        return {
      ...
      methods: {
        getDataDosen() {
          axios.get(BaseUrl + "/dosen").then((res) => {
            this.data_dosen = res.data;
          });
        },
        deleteMatakuliah(event, dosenId) {
          if (confirm("Apakah anda yakin ingin menghapus data ini ?")) {
            event.target.innerText = "Deleting";
            axios.delete(BaseUrl + `matakuliah/${dosenId}`).then((res) => {
