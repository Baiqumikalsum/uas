<template>
  <div class="md:flex-shrink-0 mx-auto mt-3">
    <img
      class="rounded-md md:w-25 lg:w-25 lg:h-60 md:h-60"
      src="~/assets/images/logokampus.png"
      alt="Woman paying for a purchase" />
  </div>
  <div class="container mx-auto bg-green-100 rounded-lg mt-10">
   
    <div class="md:flex px-4">
      <div class="md:flex-shrink-0 mx-auto mt-3">
        <img
          class="rounded-md md:w-62 lg:w-62 lg:h-80 md:h-80"
          src="~/assets/images/umy.jpg"
          alt="Woman paying for a purchase" />
      </div>
      <div class="mt-2 md:mt-0 md:ml-6 py-28">
        <h1 class="text-3xl font-semibold text-slate-800 md:text-2xl">
          Bq Umi Kalsum
          <br />
          TEKNIK INFORMATIKA
        </h1>
        <p class="mt-2 text-slate-700 text-justify lg:w-5/6 md:w-2/3">
        
        </p>
      </div>
     
    </div>
  </div>
</template>

<script setup></script>

<style lang="scss" scoped></style>
